...
class BasicAwaitable(Awaitable[T]):
    def __init__(self, awaitable: Awaitable[T]) -> None:
        self._awaitable = self._awaitable(awaitable)

    @staticmethod
    def _awaitable(awaitable: Awaitable[T]) -> Awaitable[T]:
        if isinstance(awaitable, BasicAwaitable):
            return awaitable._awaitable
...
class AwaitableMonad(BasicAwaitable[T]):
    ...
    def pure(cls, value: T) -> Self:
        ...
        return cls(_awaitable())

    @classmethod
    def fun(cls, f: Callable[[T], Awaitable[U]]) -> Callable[[T], AwaitableMonad[U]]:
        @wraps(f)
        def wrapped(x: T, /) -> AwaitableMonad[U]:
            return AwaitableMonad(f(x))
...
class BasicAsyncIterator(AsyncIterator[T]):
    def __init__(self, iterable: AsyncIterableLike[T]) -> None:
        self._iterator = self._async_iterator(iterable)

    def _async_iterator(self, iterable: AsyncIterableLike[T]) -> AsyncIterator[T]:
        if isinstance(iterable, AsyncIteratorMonad):
            return iterable._iterator
...
class BasicAsyncContextManager(AbstractAsyncContextManager[T]):
    def __init__(self, context_manager: AsyncContextManagerLike[T]) -> None:
        self._context_manager = BasicAsyncContextManager._async_context_manager(
            context_manager
        )

    @staticmethod
    def _async_context_manager(
        context_manager: AsyncContextManagerLike[T],
    ) -> AbstractAsyncContextManager[T]:
        if isinstance(context_manager, BasicAsyncContextManager):
            return context_manager._context_manager
